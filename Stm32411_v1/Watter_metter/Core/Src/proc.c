...
void Send_Time(void) {
	...
	HAL_RTC_GetTime(&hrtc, &sTime,              RTC_FORMAT_BIN);//RTC_FORMAT_BIN); RTC_FORMAT_BCD  //RTC_FORMAT_BCD); //
	HAL_RTC_GetDate(&hrtc, &sDate,              RTC_FORMAT_BIN);                                   //RTC_FORMAT_BCD); //
	HAL_RTC_GetAlarm(&hrtc,&sAlarm,RTC_ALARM_A, RTC_FORMAT_BIN);                                   //RTC_FORMAT_BCD); //
...
//AL_StatusTypeDef HAL_RTC_SetDate(RTC_HandleTypeDef *hrtc, RTC_DateTypeDef *sDate, uint32_t Format);
	...
	HAL_RTC_SetDate(&hrtc, &sDate, RTC_FORMAT_BCD);//RTC_FORMAT_BIN);
}
//====================================
uint32_t IntToHex(uint32_t iDate) {
	char buf[10];
